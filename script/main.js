...
fileUploader.addEventListener('change', (event) => {
  ...
  function validator() {
    ...
    if (fileUploader.files.length > 5) {
        output.innerHTML = `Maximum number of files 5`;
    ...
    for (const file of fileUploader.files) {
      ...
      if (file.size > 5242880) {
        output.innerHTML = `Maximum file size 5Mb. ${file.name}: ${(file.size/1048576).toFixed(2)} Mb`;
        output.style.color = 'red';
        btnSubmit.disabled = true;
        btnSubmit.classList.add('disabled');
        return;
      ...
      && file.type != 'image/gif') { 
        output.innerHTML = `Not supported file type: ${file.type}`;
        output.style.color = 'red';
        btnSubmit.disabled = true;
        btnSubmit.classList.add('disabled');
        return;
    ...
    output.innerHTML = `File(s) are ready for upload`;
  ...
  form.onsubmit = function () {
     ...
     if (form.formats.value == 'null') {
         output.innerHTML = 'Choose a format to save';
         output.style.color = 'red';
         return false;
     }
